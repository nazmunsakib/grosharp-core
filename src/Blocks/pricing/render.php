	...
	<div class="gs-container">
		...
		<div class="grid gap-6 lg:grid-cols-3">
			...
			<article class="gs-reveal relative flex flex-col rounded-[24px] bg-[#654cff] p-8 md:p-10">
				...
				<div class="mt-4">
					<span class="block font-heading text-[40px] font-bold leading-none tracking-[-0.03em] text-white">
						<?php echo esc_html( $item['price'] ?? '' ); ?>
					</span>
					<?php if ( ! empty( $item['period'] ) ) : ?>
					<span class="mt-1.5 block font-body text-[13px] font-medium text-white/55">
				...
				<!-- Divider -->
				<div class="my-7 h-px bg-white/15"></div>

				<!-- Features list -->
				<?php if ( ! empty( $item['features'] ) ) : ?>
				<ul class="space-y-3 flex-1">
					<?php foreach ( $item['features'] as $feature ) : ?>
					<li class="flex items-start gap-3 font-body text-[14.5px] leading-snug text-white/85">
						<span class="mt-0.5 flex h-5 w-5 flex-none items-center justify-center rounded-full bg-white/20 text-[11px] font-bold text-white" aria-hidden="true">✓</span>
			...
			<article class="gs-reveal flex flex-col rounded-[24px] border border-black/[0.08] bg-white p-8 shadow-[0_4px_20px_rgba(101,76,255,0.05)] md:p-10">
				...
				<!-- Divider -->
				<div class="my-7 h-px bg-black/[0.06]"></div>
